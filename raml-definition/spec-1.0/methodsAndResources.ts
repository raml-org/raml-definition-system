...
export class ResourceTypeRef extends Sys.Reference<ResourceType>{
    ...
    $resourceType = [MetaModel.customHandling(),MetaModel.description("Returns referenced resource type")]
}

export class TraitRef extends Sys.Reference<Trait>{

    trait:Trait
    $trait = [MetaModel.customHandling(),MetaModel.description("Returns referenced trait")]
}
export class SecuritySchemePart extends MethodBase {
    ...
    $queryParameters=[
        MetaModel.markdownDescription("Query parameters, used by the scheme in order to authorize the request. Mutually exclusive with queryString. See [[raml-10-spec-query-parameters-in-a-query-string|Query Parameters]] section."),
...
//    $requiredSettings=[MetaModel.description("You may declare settings needed to use this type of security security schemes"),
...
//
//    $=[MetaModel.declaresSubTypeOf("SecuritySchemeSettings"),MetaModel.description("Security scheme type allows you to contribute your own security scheme type with settings and optinal configurator for " +
...
export class OAuth2SecuritySchemeSettings extends  SecuritySchemeSettings{
    ...
    $authorizationGrants=[
        MetaModel.required(),
        MetaModel.description(`A list of the Authorization grants supported by the API as defined in RFC6749 [RFC6749] Sections 4.1, 4.2, 4.3 and 4.4, can be any of:
 - authorization_code
 - password
 - client_credentials
 - implicit
 - refresh_token.`),
        MetaModel.markdownDescription("A list of the Authorization grants supported by the API as defined in RFC6749 [RFC6749] Sections 4.1, 4.2, 4.3 and 4.4, can be any of:<br>* authorization_code<br>* password<br>* client_credentials<br>* implicit<br>* refresh_token."),
    ]
...
export class PassThroughSecuritySchemeSettings extends  SecuritySchemeSettings{
    ...
    $queryParameterName = [MetaModel.description("Name of query parameter used to pass key")]

    headerName:string
    $headerName = [MetaModel.description("Name of header used to pass key")]
...
export class SecuritySchemeRef extends Sys.Reference<AbstractSecurityScheme>{
    ...
    $securitySchemeName = [MetaModel.customHandling(), MetaModel.description("Returns the name of security scheme, this reference refers to.") ]

    securityScheme:AbstractSecurityScheme
    $securityScheme = [MetaModel.customHandling(),MetaModel.description("Returns AST node of security scheme, this reference refers to, or null.")]
}
...
It describes the minimum set of properties which any processing application MUST provide and validate if it chooses to implement the security scheme. Processing applications MAY choose to recognize other properties for things such as token lifetime, preferred cryptographic algorithms, and more.`)]
    ...
    $=[MetaModel.description("Declares globally referable security scheme definition"),MetaModel.actuallyExports("$self"),MetaModel.referenceIs("settings")]
}
...
export class Trait extends MethodBase implements Sys.DeclaresDynamicType<Trait>{
    ...
    parametrizedProperties:models.TypeInstance
...
export class ResourceType extends ResourceBase implements Sys.DeclaresDynamicType<ResourceType>{
    ...
    usage:string
    $usage = [ MetaModel.description("Instructions on how and when the resource type should be used.") ]
    $=[MetaModel.inlinedTemplates(),MetaModel.allowQuestion()]
